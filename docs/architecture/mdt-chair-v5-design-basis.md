...
1. **Chinese Thoracic Society ILD-MDD consensus (2023)**  
   ...
   - Design consequence: an absent or inadequate source is an assessment boundary/evidence need, not a positive specialty assessment and not a negative finding.
...
The sources above support preserving professional level, diagnostic confidence, and missing-data boundaries. The following conflict model is a conservative clinical implementation of those constraints, not a quotation from a guideline:

- A `direct_contradiction` requires the same atomic proposition, object, timeframe, evidence conditions, and professional level, with one assessable specialty assessment directly affirming it and another directly denying it.
- A `decision_relevant_discordance` requires at least two specialties to offer assessable preferred assessments for the same MDT decision target, where the preferred interpretations cannot both be adopted in the current integrated conclusion and the choice would change diagnosis, confidence, testing, or treatment direction.
- Possible alternatives that can coexist, an assessable opinion paired with a non-assessable opinion, mismatched timeframes or evidence scopes, missing material, and requests for explanation are not conflicts. They remain integrated alternatives, boundaries, evidence needs, or questions respectively.
- A tentative claim cannot become a decision-relevant discordance merely because another specialty names a different tentative claim. The discordance test requires preferred primary assessments whose source status is `supported` or `favored`.
- Specialty response, answer completeness, requester review, and discussion closure are separate states. A boundary answer can complete the medical content while remaining unreviewed until the requesting specialty accepts it.
- Evidence coverage is determined from the information actually supplied by cited assessments, not from the fact that a specialty spoke or was cited.
