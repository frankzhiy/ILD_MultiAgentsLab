...
## Persistent guideline knowledge

The pulmonology, thoracic-radiology, pathology, and rheumatology agents use the canonical guideline registry
at `data/guidelines/catalog.yaml`. YAML rules refer to stable `guideline_id` values; display titles,
PDF pages, and exact quotations are resolved from the local index rather than generated by the LLM.

Build the index once in the same environment used to run the agents:

```bash
/opt/anaconda3/envs/multiAgent/bin/python scripts/guidelines/build_guideline_index.py
```

The command parses the PDFs, embeds their page-level chunks with `BAAI/bge-m3`, and persists Qdrant
local-mode data under `data/guidelines/index/`. No database server or Homebrew service is required.
An unchanged second run exits without parsing or embedding; when one PDF changes, only that document
is re-parsed and re-embedded. Agent execution fails clearly when the index is missing or stale.

In networks where Hugging Face is slow, the same official BAAI weights can be downloaded from
[BAAI's ModelScope repository](https://modelscope.cn/models/BAAI/bge-m3) into
`data/guidelines/models/bge-m3`:

```bash
/opt/anaconda3/envs/multiAgent/bin/python -m pip install -e '.[guideline-model-download]'
/opt/anaconda3/envs/multiAgent/bin/python scripts/guidelines/download_embedding_model.py
```

The index builder automatically prefers this local copy. The model and generated index remain
local and are excluded from Git.

Inspect retrieval independently of an LLM call:

```bash
/opt/anaconda3/envs/multiAgent/bin/python scripts/guidelines/search_guidelines.py \
  "ILD 诊断信度和未分类 ILD 如何定义"
```

At runtime, each stage receives only its configured retrieved chunks. The LLM may cite only those
`chunk_id` values. Local validation then fills the canonical title, organization, section, PDF page,
exact passage, and source file. Stage traces record the query, candidates, scores, and citations used.
Specialty HTML reports keep patient evidence, conflicting evidence, related context, and guideline
knowledge visually separate and link citations to the corresponding local PDF page.
...
## Pathology ILD Consultation Agent

The pathology agent reuses the same evidence and guideline infrastructure while maintaining a
pathology-specific state. It runs three initial stages (specimen reconstruction, morphologic
assessment, consult formulation) and three discussion stages (evidence mapping, state update,
chair response). The current implementation is text-only: it distinguishes formal reports,
excerpts, paraphrases, and absent material, and never claims direct slide review.

Run it after semantic-graph outputs are available:

```bash
python3 scripts/run/run_pathology_agent.py
```

Configuration and guideline guardrails are in `configs/agents/pathology/agent.yaml`.
