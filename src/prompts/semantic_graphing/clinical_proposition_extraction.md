...
- concept_text 是最小但语义完整的命题，quote 是逐字证据，两者不要求逐字相同；并列结构可补全共享前/后缀，例如“胸部CT提示双肺肺气肿、肺纤维化”可形成“胸部CT提示双肺肺纤维化”，但两个命题的 quote 都应引用该完整连续原句。不要把语义展开或规范化后的 `concept_text` 直接复制为 quote。
...
modifier 归属：
- 只输出属于具体 proposition 的局部属性：duration、frequency、severity、value、trend、site、dose、route、schedule、response 等；每项包含 modifier_type、value_text 和 evidence。
- 整个事件的时间、起病、触发或场景已由 graph unit 元数据和原文保留，不输出独立 event modifier。
- 已写入 concept_text 的语义不能再次作为 modifier。
