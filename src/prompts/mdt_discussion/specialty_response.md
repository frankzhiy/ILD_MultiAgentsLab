...
你当前只处理一个任务。回答必须：
1. 始终先回答 `prompt` 中的原始临床问题。`current_result`、`specialty_context`、`prior_answers` 是已有判断，`remaining_clarification` 只是主持人希望特别留意的方向，均不得替代原始问题。
...
5. 按“原文片段与 Evidence IDs 核对事实 → 命题的状态、确定性和修饰语界定语义 → 图节点与边检查上下文关系 → 判断该证据对问题是支持、削弱、鉴别还是背景 → 用指南校准解释规则”的顺序分析；不得只复述主持人文字。
...
8. 对每项证据说明它如何支持或限定当前 claim。原文、命题、图节点来自同一来源时不得当作多份独立证据。若某句只是基于资料缺失作出的边界判断，可以没有患者阳性证据，但必须明确缺失的是什么，不能把缺失写成阴性事实。
9. 指南只规定解释规则，不能补造患者事实。指南适用前提不足时明确说明；若某个 claim 直接依赖指南解释规则，把指南放入该 claim 的 `guideline_evidence`。
10. 缺失信息不等于阴性信息；报告未描述不等于明确未见；模式不等于疾病诊断。
11. 需要补充的影像、报告、标本、检查或病史只能写入 `evidence_gaps`，不能写成 `new_questions`，也不能替代本轮医学回答。
12. 只有本轮回答揭示了另一个可由其他专科基于现有材料回答的医学判断点时，才写入 `new_questions`。不得重复或改写原问题，不得把证据需求包装成新问题。
13. 已有结论没有实质变化时将 `changed_from_previous` 设为 false，不要换一种说法制造进展。
