...
输入说明：
- `orientation_units`覆盖全部病例unit，仅用于识别临床触发、检查目的和问题优先级。它们不能自动成为影像事实。
- `evidence_units`只保留候选影像unit的proposition级内容。只有`thoracic_imaging_eligible=true`的statement可以进入检查来源和reported statement。
- `excluded_radiology_candidates`是上游虽标记影像科、但程序判定未包含胸部CT/HRCT/CTPA/胸片信号的unit。肺功能、心脏超声、下肢超声不得建成胸部影像检查。
...
- examination和reported statement必须引用`thoracic_imaging_eligible=true`的proposition。
- orientation_units只提供病例定向文本，不提供可引用的proposition ID；不得猜测ID。context_evidence仅在evidence_units中有可见proposition可支持临床触发时填写，否则留空。
