...
from src.schemas.semantic_graphing.graph_unit import DocumentGraphUnits, MdtSpecialty
...
def build_specialty_case_input(
    ...
    *,
    ...
    """Project semantic outputs into one specialty's explicit evidence contract."""
    ...
    frame_segments = _index_unique(primary_frames.segments, "segment_id", "primary frames")
    proposition_segments = _index_unique(
        propositions.segments, "segment_id", "clinical propositions"
    )
    validation_segments = _index_unique(
        validations.segments, "segment_id", "proposition validation"
    )
    local_graph_segments = _index_unique(
        local_graphs.segments, "segment_id", "local graphs"
    )
        ...
        for unit_index, unit in enumerate(graph_segment.graph_units, start=1):
            ...
            merged_units.append(
                SpecialtyUnitInput(
                    ...
                    evidence_role=role,
                    may_support_diagnostic_claim="diagnostic_support"
                    in _ALLOWED_USES[role],
...
def _discover_case_id(run_dir: Path) -> str:
    ...
    ids = sorted(path.name[: -len(suffix)] for path in run_dir.glob(f"*{suffix}"))
    if len(ids) != 1:
        raise ValueError(f"Expected exactly one case in {run_dir}; found {ids}.")
    return ids[0]
...
def _evidence_role(
    specialties: list[MdtSpecialty], target: MdtSpecialty
) -> EvidenceRole:
